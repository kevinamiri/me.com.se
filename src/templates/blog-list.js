    ...
    <Layout location={props.location}>
      ...
      {posts.map(({ node }) => {
        ...
        for (const [index, value] of elements.entries()) {
          items.push(
            <span
              className="rounded bg-gray-400 hover:bg-orange-300 px-1 py-1 text-xs mr-3 text-white"
              key={index}
            >
              <Link className="shadow-none" to={`/tags/${kebabCase(value)}`}>
                {value}{" "}
              </Link>{" "}
            </span>
          )
      ...
      <div className="flex-row sm:flex-col md:flex-row-reverse lg:flex-col-reverse xl:flex-row my-12">
        <ul className="flex justify-between">
          ...
          {!isLast && (
            <Link
              className="bg-gray-400 hover:bg-orange-300 py-2 px-4 rounded-r text-black-600 shadow-none"
              to={nextPage}
              rel="next"
            >
              Next Page →
